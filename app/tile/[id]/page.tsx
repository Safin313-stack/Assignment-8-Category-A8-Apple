...
async function getTile(id: string) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_JSON_SERVER_URL}/tiles/${id}`, { cache: "no-store" });
...
export default async function TileDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center px-4">
        <div>
          ...
          <Link href="/all-tiles" className="text-[#c9a84c] underline font-body text-sm">← Back to Gallery</Link>
        </div>
      </div>
    );
  }
            ...
            {tile.tags && (
              <div className="flex flex-wrap gap-2 mb-8">
                {tile.tags.map((tag: string) => (
                  <span
                    key={tag}
                    className="px-3 py-1 text-xs border border-[#c9a84c]/30 text-[#c9a84c]/80 tracking-widest uppercase font-body"
                  >
                    {tag}
                  </span>
                ))}
