...
export default function RegisterPage() {
  ...
  const fields = [
    ...
    { key: "image", label: "Photo URL (optional)", type: "url", placeholder: "https://..." },
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-20"
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-2xl mt-4">Create Account</h1>
          <p className="text-sm text-[#f0ece0]/50 mt-2 font-body">Join the TileVerse community</p>
        </div>
        ...
        <div className="border border-[#c9a84c]/20 p-8 bg-[#1a1915]">
          <form onSubmit={handleSubmit} className="space-y-5">
            {fields.map(({ key, label, type, placeholder }) => (
              <div key={key}>
                <label className="text-xs tracking-widest uppercase text-[#f0ece0]/50 font-body block mb-2">{label}</label>
                <input
                  type={type}
                  value={form[key]}
                  required={key !== "image"}
          ...
          <p className="text-center text-sm text-[#f0ece0]/50 mt-6 font-body">
            ...
            <Link href="/login" className="text-[#c9a84c] hover:underline">Login</Link>
          </p>
        </div>
      </div>
    </div>
